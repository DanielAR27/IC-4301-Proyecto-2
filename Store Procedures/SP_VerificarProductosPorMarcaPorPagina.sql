USE DB_Proyecto;
GO
CREATE OR ALTER PROCEDURE VerificarProductosPorMarcaPorPagina
    @NombreMarca NVARCHAR(100), -- Parámetro de entrada para el nombre de la marca
...
BEGIN
    BEGIN TRY
        ...
        IF EXISTS (
            SELECT 1
            FROM Productos P
            JOIN Marcas M ON P.MarcaID = M.MarcaID -- Relación con la tabla de marcas
            WHERE P.Stock > 0 -- Verificar que el stock sea mayor que cero
              AND M.Nombre = @NombreMarca -- Filtrar por el nombre de la marca
